...
# -*- coding: utf-8 -*-
"""
diag_bind_log.py — 口令绑定日志诊断工具

解密并展示 bind.py 写入的加密绑定日志，方便排查口令绑定、续期、会场链接相关问题。

用法：
  python diag_bind_log.py              # 展示最近 100 条
  python diag_bind_log.py --tail 20    # 展示最近 20 条
  python diag_bind_log.py --all        # 展示全部日志
"""
...
# ── 路径常量 ──────────────────────────────────────────────────────────
BIND_LOG_FILE = Path(tempfile.gettempdir()) / "fenxiao" / "venue_bind.log"
...
def main():
    parser = argparse.ArgumentParser(
        description="解密并展示 bind.py 口令绑定日志"
    )
    ...
    if not BIND_LOG_FILE.exists():
        print(f"[INFO] 日志文件不存在：{BIND_LOG_FILE}")
        print("       尚未产生任何口令绑定操作记录，或日志路径已更改。")
        sys.exit(0)
    ...
    with open(BIND_LOG_FILE, encoding="utf-8", errors="replace") as f:
        lines = f.readlines()
    ...
    print(f"{'─' * 60}")
    print(f"  绑定日志诊断  |  文件：{BIND_LOG_FILE}")
